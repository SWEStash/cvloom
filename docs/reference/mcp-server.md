...
**Every tool that resolves a profile also returns `warnings`** — a list of strings, empty when there are none. These are the non-fatal advisories resolution produced: a data file that is missing, a `select` clause that matched nothing, an overlay whose `match` hit no entry, a locale key falling back to `en`, a work range whose duration could not be computed. Each names a silent failure — the build succeeds and the output is quietly wrong — so an empty list and an absent key must not be read the same way. `diff_profiles` splits them into `warnings_a` / `warnings_b`, since which profile produced a warning is the point of a comparison. The CLI prints these to stderr; until they were returned here an agent had no way to see them at all.
...
| `build_cv` | `profile?` (default `"general"`), `public?` (bool), `skip_pdf?` (bool), `project_root?` | `{html_path, pdf_path, words, pages, section_word_counts, warnings}` | Builds the CV for the given profile. Set `public` to use placeholder contact info. |
...
| `validate_data` | `project_root?` | `{valid, locale, warnings}`, or `{valid: false, errors: [...]}` when `cvloom.yaml` itself is invalid (no locale resolved yet) | Runs schema validation against `cvloom.yaml` and all data files in the project. Config is checked first: a project whose config is invalid cannot build, so reporting the data as valid would be misleading. `locale` names which project's settings were applied — not inferable from `valid: true` alone. |
| `export_json_resume` | `profile?` (default `"general"`), `public?` (bool, default **true**), `project_root?` | `{document, warnings}` — the JSON Resume document under `document` | Resolves the given profile and exports it in [JSON Resume](https://jsonresume.org) format. **PII fence:** `public` defaults to `true` (email/phone stripped); pass `public=false` to include real contact PII. |
| `check_cv` | `profile?` (default `"general"`), `rule_ids?` (string array), `project_root?` | `{locale, rules_run, rules_total, rules_skipped: [...], findings: [...], warnings}`; each finding has `rule_id`, `category`, `severity`, `section`, `entry`, `message`, `fix_hint` | Runs the writing lint on the resolved profile; each finding carries a `category` (`writing`/`structure`/`ats-parse`). Optionally filter by rule IDs. Findings are wrapped in coverage because not every rule runs in every language: an empty `findings` list under a locale that skipped rules is a weaker result than one under a locale that ran them all. |
| `trim_report` | `profile?` (default `"general"`), `target_pages?` (int, default 3), `project_root?` | `{total_words, estimated_pages, words_to_cut, sections: [...], recommendations: [...], warnings}` | Per-section word count breakdown with actionable trim recommendations. |
| `diff_profiles` | `profile_a` (string), `profile_b` (string), `project_root?` | `{template_a, template_b, sections_only_in_a, sections_only_in_b, entries_only_in_a, entries_only_in_b, word_count_a, word_count_b, highlight_count_a, highlight_count_b, warnings_a, warnings_b}` | Compares two profiles: sections, entries, word counts, and highlight counts. |
| `match_jd` | `jd_text` (string), `profile?` (default `"general"`), `project_root?` | `{coverage, jd_word_count, matched: [...], gaps: [...], top_jd_keywords: [...], warnings}` | Keyword gap analysis comparing CV content against a job description. |

### AI tools (require `--extra ai` and a provider)

| Tool | Parameters | Returns | Description |
|------|-----------|---------|-------------|
| `ai_review_cv` | `profile?` (default `"general"`), `project_root?` | `{overall_band, sections: [...], top_priorities: [...], warnings}` | AI section assessment banded `strong`/`adequate`/`needs work`, with strengths, weaknesses, and improvement suggestions. `overall_band` is the worst section, computed by cvloom. |
| `ai_generate_cover` | `profile?` (default `"general"`), `jd_text` (string), `project_root?`, `body_only?` (default `false`) | `{letter, word_count, key_alignments: [...], context_notes: [...], body_only, warnings}` | AI-generated tailored cover letter from CV and job description text. With `body_only`, `letter` is body paragraphs alone, for a profile's `job_context.notes` — a `cover-letter/*` template renders the greeting, closing and signature itself. |
| `ai_suggest_improvements` | `profile?` (default `"general"`), `role?` (string), `project_root?` | `{suggestions: [...], missing_skills: [...], summary, warnings}` | AI content improvement suggestions for a target role. |
| `ai_align_to_jd` | `profile?` (default `"general"`), `jd_text` (string), `project_root?` | `{alignment_band, narrative, repositioning: [...], tone_gaps: [...], strengths: [...], warnings}` | Qualitative AI analysis of CV-to-JD alignment — tone, framing, and repositioning actions. |
...
7. **Export.** Call `export_json_resume` to produce a JSON Resume file for uploading to job boards — the document is under `document`, so write that rather than the whole response.
